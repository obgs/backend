...
func (gq *GroupQuery) QuerySettings() *GroupSettingsQuery {
	query := &GroupSettingsQuery{config: gq.config}
...
func (gq *GroupQuery) QueryMembers() *GroupMembershipQuery {
	query := &GroupMembershipQuery{config: gq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) IDs(ctx context.Context) ([]guidgql.GUID, error) {
	var ids []guidgql.GUID
...
func (gq *GroupQuery) loadSettings(ctx context.Context, query *GroupSettingsQuery, nodes []*Group, init func(*Group), assign func(*Group, *GroupSettings)) error {
	...
	query.Where(predicate.GroupSettings(func(s *sql.Selector) {
		s.Where(sql.InValues(group.SettingsColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "group_settings" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (gq *GroupQuery) loadApplications(ctx context.Context, query *GroupMembershipApplicationQuery, nodes []*Group, init func(*Group), assign func(*Group, *GroupMembershipApplication)) error {
	...
	query.Where(predicate.GroupMembershipApplication(func(s *sql.Selector) {
		s.Where(sql.InValues(group.ApplicationsColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "group_applications" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
