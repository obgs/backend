...
func (mq *MatchQuery) FirstIDX(ctx context.Context) guidgql.GUID {
...
func (mq *MatchQuery) OnlyIDX(ctx context.Context) guidgql.GUID {
...
func (mq *MatchQuery) IDsX(ctx context.Context) []guidgql.GUID {
	ids, err := mq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (mq *MatchQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Match, error) {
	...
	for name, query := range mq.withNamedPlayers {
		if err := mq.loadPlayers(ctx, query, nodes,
			func(n *Match) { n.appendNamedPlayers(name) },
			func(n *Match, e *Player) { n.appendNamedPlayers(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (mq *MatchQuery) loadPlayers(ctx context.Context, query *PlayerQuery, nodes []*Match, init func(*Match), assign func(*Match, *Player)) error {
	...
	byID := make(map[guidgql.GUID]*Match)
	nids := make(map[guidgql.GUID]map[*Match]struct{})
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(player.FieldID), joinT.C(match.PlayersPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(match.PlayersPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(match.PlayersPrimaryKey[0]))
...
func (mq *MatchQuery) WithNamedPlayers(name string, opts ...func(*PlayerQuery)) *MatchQuery {
	...
	if mq.withNamedPlayers == nil {
		mq.withNamedPlayers = make(map[string]*PlayerQuery)
	}
	mq.withNamedPlayers[name] = query
	return mq
}
